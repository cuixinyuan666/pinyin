...
3. **拼音语音交互（按《汉语拼音方案》标准读音，v1.3.2 起改为「以汉字驱动 TTS」发音）**：
   - **发音修复**：此前把拉丁拼音串（如 `b`、`sh`、`ao`）直接交给 TTS，中文引擎会按**英文字母**读出。现在一律给 TTS 传**汉字**，从根本上杜绝英文读音：
     - 声母 → 朗读其「呼读音」对应汉字（如 `t`→特、`x`→西、`sh`→诗、`j`→鸡；刻意避开多音字：`b`→波、`d`→德、`l`→了）。
     - 韵母 → 朗读该韵母对应声调的**例字**（如「少」的 ao 三声→袄、「牛」的 iu 二声→油）；`j/q/x/y + u` 按 ü 读（去→玉、鱼→鱼、雨→雨），`j/q/x/y + un` 按 ün 读（云→云）。
     - 缺少合适例字的「韵母+声调」自动**回退朗读本字**，确保永远是正确的中文发音。
   - **声调**：点击声调按钮直接朗读**当前汉字本身**（即本字的真实声调），不再统一用「a（啊）」示范（v1.3.3）。
   - 每道题完整作答后自动整段拼读：**声母例字 → 本字声调的韵母例字 → 本字**（如「少」→ 诗 + 袄 + 少）。
   - **界面修复（v1.3.4）**：韵母按钮改为等宽平分整行 + 文字自动缩放，较长韵母（如 `uang`/`iao`/`uan`/`iang`）不再被裁切、只显示局部；复习完成后点「先不玩，继续答题」可正常返回下一轮练习（此前遮罩未隐藏导致无法跳转）。
...
- 拼音拆分为「声母 / 韵母 / 声调」三行独立选择。
- **发音以汉字驱动 TTS**：内置「声母→呼读音汉字」映射与「韵母×四声→例字」映射表，所有朗读都只把**汉字**交给 `TextToSpeech`，从根本上避免中文引擎把拉丁拼音串读成英文字母；无例字的组合回退朗读本字。
