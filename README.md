...
3. **拼音语音交互（按《汉语拼音方案》标准读音，v1.3.1 起全部使用带调拼音朗读）**：
   - 点击任一拼音字母即朗读该字母的**标准拼音读法**，而非英文字母读音。声母使用带调呼读音（如 `t`→tē 特、`x`→xī 希、`sh`→shī 诗），韵母**贴合当前汉字的实际声调**带调朗读（如「少」选韵母 ao → ǎo、「牛」选 iu → iú）。
   - 声调跟随对应汉字的韵母发声（如「少」的第三声读作「ǎo」，不再固定读「啊」）。
   - 每道题完整作答后自动整段拼读：**声母呼读音 → 带本字声调的韵母 → 汉字**（如「下」→ xī + ià + 下，「少」→ shī + ǎo + 少）。
   - 中文 TTS 对带调拼音（ā á ǎ à）能准确按汉字发音朗读，彻底避免无调拼音被误读成英文。
