...
3. **拼音语音交互（按《汉语拼音方案》标准读音，v1.3.2 起改为「以汉字驱动 TTS」发音）**：
   - **发音修复（v1.3.2）**：此前把拉丁拼音串（如 `b`、`sh`、`ao`）直接交给 TTS，中文引擎会按**英文字母**读出。现在一律给 TTS 传**汉字**，从根本上杜绝英文读音：
     - 声母 → 朗读其「呼读音」对应汉字（如 `t`→特、`x`→西、`sh`→诗、`j`→鸡）。
     - 韵母 → 朗读该韵母对应声调的**例字**（如「少」的 ao 三声→袄、「牛」的 iu 二声→油）。
     - 缺少合适例字的「韵母+声调」自动**回退朗读本字**，确保永远是正确的中文发音。
   - 声调跟随对应汉字的韵母发声（点击声调按钮朗读本字韵母在该声调下的例字读音）。
